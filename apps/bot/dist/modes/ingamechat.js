...
export class InGameChatMode extends BaseBotMode {
    name = 'ingamechat';
    description = 'In-game chat relay (public chat + whisper)';
    requiredEnv = [
        'SERVER_IP',
        'SERVER_PORT',
        'SESSION_ID',
    ...
    async flushMessageBuffer() {
        ...
        if (this.messageBuffer.length === 0 && players.length === 0)
            return;
    ...
    async pollOutbox() {
        try {
            const res = await fetch(`${this.backendUrl}/api/ingame-chat/bot-callback?sessionId=${this.sessionId}`, {
    ...
    processSendQueue() {
        ...
        if (!this.client?.isConnected())
            return;
        const now = Date.now();
        if (now - this.lastSendTime < WHISPER_INTERVAL_MS)
            return;
