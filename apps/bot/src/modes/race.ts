...
export class RaceMode extends BaseBotMode {
  ...
  private raceMaps: RaceMap[] = []
  private currentStep = 0
  private currentMap = ''
  private stopping = false
  private reconnectAttempts = 0
  private isFirstConnect = true
  private connectedClient: TeeworldsClient | null = null
  ...
  async run(): Promise<void> {
    ...
    client.onDisconnect((reason: string) => {
      ...
      this.log(
        `[Race] Disconnected: ${reason}. Reconnecting (attempt ${this.reconnectAttempts}/${MAX_RECONNECT_ATTEMPTS})...`,
      )
   ...
   */
  private attachHandlers(client: TeeworldsClient): void {
    ...
    if (rawClient) {
      rawClient.on('map_change', (...args: unknown[]) => {
  ...
  private getExpectedMap(): RaceMap | undefined {
    return this.raceMaps.find((m) => m.position === this.currentStep)
  }

  private async sayWithDelay(client: TeeworldsClient, message: string): Promise<void> {
   ...
   */
  private parseFinishMessage(text: string): { playerName: string; finishTime: number } | null {
   ...
   */
  private async handleServerMessage(client: TeeworldsClient, text: string) {
    ...
    if (expectedMap && this.currentMap) {
      ...
      if (!isCorrectMap) {
        ...
        await this.sayWithDelay(
          client,
          `${trimmedName} finished "${this.currentMap}" but the current race map is "${expectedMap.mapName}"!`,
        )
        return
      }
    }
  ...
  private startStatusPolling(client: TeeworldsClient): void {
    this.statusPollTimer = setInterval(async () => {
      if (this.stopping || this.pollingInFlight) return
      this.pollingInFlight = true

      try {
        const status = await this.api!.getRaceStatus(this.config.RACE_ID) as any
        if (!status) { this.pollingInFlight = false; return }
   ...
   */
  private async announceGameEnd(client: TeeworldsClient, status: any): Promise<void> {
    if (!client.isConnected()) {
