...
export class TestMode extends BaseBotMode {
  ...
  readonly requiredEnv = ['SERVER_IP', 'SERVER_PORT', 'SESSION_ID', 'BACKEND_URL', 'BACKEND_SECRET']
  ...
  private async pollOutbox(): Promise<void> {
    try {
      const res = await fetch(
        `${this.backendUrl}/api/admin/container-test/bot-callback?sessionId=${this.sessionId}`,
        {
