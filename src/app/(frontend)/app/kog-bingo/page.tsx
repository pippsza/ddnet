...
function BingoLobbyContent() {
  ...
  const { data: lobbyData, isLoading: lobbyLoading } = useSWR(
    buildUrl('/api/kog-bingo?where[isPublic][equals]=true&where[gameStatus][equals]=waiting&sort=-createdAt&depth=2'),
    fetcher,
    { refreshInterval: 5000 },
  )
  const { data: myData, isLoading: myLoading, mutate: mutateMyGames } = useSWR('/api/game/my-games?type=kog-bingo', fetcher, { refreshInterval: 5000 })
    ...
    <PageTransition className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link href="/app/kog-bingo/create">
    ...
    <Card className={isPendingInvite ? 'border-yellow-500/40' : isMine ? 'border-primary/30' : ''}>
      <CardContent className="flex items-center justify-between p-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href={`/app/kog-bingo/${game.id}`}>
