...
function RaceLobbyContent() {
  ...
  const { data: lobbyData, isLoading: lobbyLoading } = useSWR(
    buildLobbyUrl('/api/kog-races?where[isPublic][equals]=true&where[gameStatus][in]=waiting,ready&sort=-createdAt&depth=1'),
    fetcher,
    { refreshInterval: 5000 },
  )

  const { data: activeData, isLoading: activeLoading, mutate: mutateMyRaces } = useSWR(
    userId ? `/api/kog-races?where[teams.players.user][equals]=${userId}&where[gameStatus][in]=waiting,ready,in_progress&sort=-createdAt&depth=1&limit=50` : null,
    fetcher,
    { refreshInterval: 5000 },
  )

  const { data: pastData } = useSWR(
    userId ? buildPastUrl(`/api/kog-races?where[teams.players.user][equals]=${userId}&where[gameStatus][in]=completed,cancelled&sort=-createdAt&depth=1`) : null,
    fetcher,
    { refreshInterval: 5000 },
  )
