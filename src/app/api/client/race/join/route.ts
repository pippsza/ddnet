...
/**
 * POST /api/client/race/join — Join a race game by invite code.
...
export async function POST(req: NextRequest) {
  try {
    ...
    await payload.update({ collection: 'races', id: game.id, data: updateData })

    await payload.update({
      collection: 'users',
      id: user.id,
      data: { activeGame: { relationTo: 'races', value: game.id } },
    })
    ...
    console.error('[API] Error joining race (client):', error)
    return NextResponse.json(
