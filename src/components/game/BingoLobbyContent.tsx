...
import type { BingoPageConfig } from '@/lib/game-page-config'

const fetcher = (url: string) => fetch(url).then((r) => r.json())

interface BingoLobbyContentProps {
  config: BingoPageConfig
}
...
export function BingoLobbyContent({ config }: BingoLobbyContentProps) {
  const t = useTranslations('bingo')
  ...
  const { data: lobbyData, isLoading: lobbyLoading } = useSWR(
    buildUrl(`/api/${config.apiCollection}?where[isPublic][equals]=true&where[gameStatus][equals]=waiting&sort=-createdAt&depth=2`),
    fetcher,
    { refreshInterval: 5000 },
  )
  const { data: myData, isLoading: myLoading, mutate: mutateMyGames } = useSWR(
    `/api/game/my-games?type=${config.myGamesType}`,
    fetcher,
    { refreshInterval: 5000 },
  )
          ...
          <StaggerContainer className="grid gap-3">
            {activeGames.map((game: any) => (
              <StaggerItem key={game.id}>
                <GameCard game={game} gamePath={config.lobbyPath} isMine onCancel={() => mutateMyGames()} />
              </StaggerItem>
            ))}
          </StaggerContainer>
        </section>
        ...
        <StaggerContainer className="grid gap-3">
          {lobbyLoading ? (
            ...
            lobbyGames.map((game: any) => (
              <StaggerItem key={game.id}>
                <GameCard game={game} gamePath={config.lobbyPath} />
              </StaggerItem>
            ))
          ) : (
            <Card>
        ...
        <section className="space-y-3">
          ...
          <div className="grid gap-3">
            {paginatedPastGames.map((game: any) => (
              <GameCard key={game.id} game={game} gamePath={config.lobbyPath} />
            ))}
          </div>
          <PaginationControls
            page={pastPage}
