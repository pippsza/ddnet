...
interface RaceCreateRedirectProps {
  config: RacePageConfig
}

export function RaceCreateRedirect({ config }: RaceCreateRedirectProps) {
  ...
  useEffect(() => {
    ...
    fetch('/api/game/create', {
      ...
      .then((r) => r.json())
      .then((data) => {
        const id = data.race?.id || data.game?.id
        if (id) {
          router.replace(`${config.lobbyPath}/${id}`)
        } else {
