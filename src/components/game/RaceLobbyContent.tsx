...
import { GameCardSkeleton } from '@/components/ui/page-skeleton'
...
interface RaceLobbyContentProps {
...
export function RaceLobbyContent({ config }: RaceLobbyContentProps) {
  const t = useTranslations('race')
  ...
  const { data: lobbyData, isLoading: lobbyLoading } = useSWR(
    buildLobbyUrl(`/api/${config.apiCollection}?where[isPublic][equals]=true&where[gameStatus][in]=waiting,ready&sort=-createdAt&depth=1`),
    fetcher,
    { refreshInterval: 5000 },
  )

  const { data: activeData, isLoading: activeLoading, mutate: mutateMyRaces } = useSWR(
    userId ? `/api/${config.apiCollection}?where[teams.players.user][equals]=${userId}&where[gameStatus][in]=waiting,ready,in_progress&sort=-createdAt&depth=1&limit=50` : null,
    fetcher,
    { refreshInterval: 5000 },
  )

  const { data: pastData } = useSWR(
    userId ? buildPastUrl(`/api/${config.apiCollection}?where[teams.players.user][equals]=${userId}&where[gameStatus][in]=completed,cancelled&sort=-createdAt&depth=1`) : null,
    fetcher,
    { refreshInterval: 5000 },
  )
  ...
  const handleJoinByCode = async () => {
    ...
    try {
      ...
      router.push(`${config.lobbyPath}/${data.raceId}`)
    } catch {
    ...
    <PageTransition className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link href={`${config.lobbyPath}/create`}>
          ...
          <StaggerContainer className="grid gap-3">
            {activeRaces.map((race: any) => (
              <StaggerItem key={race.id}>
                <RaceCard race={race} gamePath={config.lobbyPath} isMine onCancel={() => mutateMyRaces()} />
              </StaggerItem>
            ))}
          </StaggerContainer>
        </section>
        ...
        <StaggerContainer className="grid gap-3">
          {lobbyLoading ? (
            ...
            lobbyRaces.map((race: any) => (
              <StaggerItem key={race.id}>
                <RaceCard race={race} gamePath={config.lobbyPath} />
              </StaggerItem>
            ))
          ) : (
            <Card>
        ...
        <section className="space-y-3">
          ...
          <div className="grid gap-3">
            {pastRaces.map((race: any) => (
              <RaceCard key={race.id} race={race} gamePath={config.lobbyPath} />
            ))}
          </div>
          <PaginationControls
            page={pastPage}
