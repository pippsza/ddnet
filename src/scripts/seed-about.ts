...
const i18n: Record<string, {
  ...
  en: {
    desc: `${APP_NAME} is a community platform built for DDNet players by a solo developer who just wanted to bring an idea to life. Play bingo, compete in races, track your stats, and connect with the community — all in one place.`,
    devTitle: 'Development', contTitle: 'Contributors',
    pippTitle: 'Lead Developer', pippDesc: `Creator and sole developer of ${APP_NAME}. Came up with the idea and built everything from scratch — backend, frontend, bot system, and infrastructure. Just a guy trying to turn a vision into reality.`,
    ...
    supportTitle: 'Support the Project', supportDesc: `${APP_NAME} is completely free and open-source. This is a passion project built by one person — any help is greatly appreciated. Whether it's contributing code, reporting bugs, suggesting features, or just spreading the word — every bit counts!`,
  ...
  ru: {
    desc: `${APP_NAME} — это платформа для игроков DDNet, созданная одним разработчиком, который просто хотел воплотить идею в реальность. Играй в бинго, участвуй в гонках, следи за статистикой и общайся с сообществом — всё в одном месте.`,
    devTitle: 'Разработка', contTitle: 'Участники',
    pippTitle: 'Ведущий разработчик', pippDesc: `Создатель и единственный разработчик ${APP_NAME}. Придумал идею и построил всё с нуля — бэкенд, фронтенд, систему ботов и инфраструктуру. Просто парень, который пытается воплотить идею в реальность.`,
    ...
    supportTitle: 'Поддержать проект', supportDesc: `${APP_NAME} полностью бесплатен и имеет открытый исходный код. Это проект одного человека — любая помощь очень ценится. Будь то написание кода, сообщения о багах, предложения по функциям или просто рассказ друзьям — каждый вклад важен!`,
  ...
  uk: {
    desc: `${APP_NAME} — це платформа для гравців DDNet, створена одним розробником, який просто хотів втілити ідею в реальність. Грай у бінго, змагайся в перегонах, відстежуй статистику та спілкуйся зі спільнотою — все в одному місці.`,
    devTitle: 'Розробка', contTitle: 'Учасники',
    pippTitle: 'Головний розробник', pippDesc: `Творець та єдиний розробник ${APP_NAME}. Придумав ідею та побудував все з нуля — бекенд, фронтенд, систему ботів та інфраструктуру. Просто хлопець, який намагається втілити ідею в реальність.`,
    ...
    supportTitle: 'Підтримати проєкт', supportDesc: `${APP_NAME} повністю безкоштовний та з відкритим кодом. Це проєкт однієї людини — будь-яка допомога дуже цінується. Чи то написання коду, повідомлення про баги, пропозиції щодо функцій або просто розповідь друзям — кожен внесок має значення!`,
  ...
  de: {
    desc: `${APP_NAME} ist eine Community-Plattform für DDNet-Spieler, gebaut von einem einzelnen Entwickler, der einfach eine Idee zum Leben erwecken wollte. Spiele Bingo, nimm an Rennen teil, verfolge deine Statistiken und verbinde dich mit der Community — alles an einem Ort.`,
    devTitle: 'Entwicklung', contTitle: 'Mitwirkende',
    pippTitle: 'Leitender Entwickler', pippDesc: `Schöpfer und alleiniger Entwickler von ${APP_NAME}. Hat die Idee entwickelt und alles von Grund auf gebaut — Backend, Frontend, Bot-System und Infrastruktur. Einfach jemand, der eine Vision Wirklichkeit werden lassen will.`,
    ...
    supportTitle: 'Projekt unterstützen', supportDesc: `${APP_NAME} ist völlig kostenlos und Open Source. Dies ist ein Herzensprojekt einer einzelnen Person — jede Hilfe wird sehr geschätzt. Ob Code beitragen, Fehler melden, Funktionen vorschlagen oder einfach weitererzählen — jeder Beitrag zählt!`,
  ...
  zh: {
    desc: `${APP_NAME} 是一个由独立开发者为 DDNet 玩家打造的社区平台，只是想把一个想法变成现实。玩宾果、参加竞速、追踪数据、与社区互动——一切尽在一处。`,
    devTitle: '开发', contTitle: '贡献者',
    pippTitle: '首席开发者', pippDesc: `${APP_NAME} 的创建者和唯一开发者。从零开始构思并打造了一切——后端、前端、机器人系统和基础设施。只是一个想把想法变为现实的人。`,
    ...
    supportTitle: '支持项目', supportDesc: `${APP_NAME} 完全免费且开源。这是一个人的热情项目——任何帮助都非常感谢。无论是贡献代码、报告问题、建议功能还是帮忙宣传——每一份贡献都很重要！`,
    contactTitle: '联系我们', contactDesc: '有问题、建议或想贡献？通过以下任何平台联系首席开发者。',
  },
  tr: {
    desc: `${APP_NAME}, sadece bir fikri hayata geçirmek isteyen tek bir geliştirici tarafından DDNet oyuncuları için oluşturulmuş bir topluluk platformudur. Bingo oynayın, yarışlara katılın, istatistiklerinizi takip edin ve toplulukla bağlantı kurun — hepsi tek bir yerde.`,
    devTitle: 'Geliştirme', contTitle: 'Katkıda Bulunanlar',
    pippTitle: 'Baş Geliştirici', pippDesc: `${APP_NAME}'un yaratıcısı ve tek geliştiricisi. Fikri buldu ve her şeyi sıfırdan inşa etti — backend, frontend, bot sistemi ve altyapı. Sadece bir vizyonu gerçeğe dönüştürmeye çalışan biri.`,
    ...
    supportTitle: 'Projeyi Destekle', supportDesc: `${APP_NAME} tamamen ücretsiz ve açık kaynaklıdır. Bu, bir kişi tarafından yapılmış tutkulu bir projedir — her türlü yardım büyük takdirle karşılanır. Kod yazma, hata bildirme, özellik önerme veya sadece başkalarına anlatma — her katkı değerlidir!`,
...
const termsSections: Record<string, { title: string; content: string }[]> = {
  en: [
    { title: '1. Acceptance of Terms', content: `By accessing and using ${APP_NAME}, you agree to be bound by these Terms of Service. If you do not agree, please do not use the platform.\n\n${APP_NAME} is a free community platform for DDraceNetwork players. These terms govern your use of the website and all associated services.` },
    ...
    { title: '4. User Content', content: `You retain ownership of content you create (forum posts, articles, etc.). By posting content, you grant ${APP_NAME} a non-exclusive license to display and distribute it on the platform.\n\nWe reserve the right to remove content that violates these terms or our community rules.` },
    { title: '5. Third-Party Services', content: `${APP_NAME} integrates with DDraceNetwork services (ddnet.org) for player data, statistics, and skin rendering. We are not affiliated with or endorsed by the DDNet team.\n\nPlayer statistics and skin data are fetched from public DDNet APIs. We do not control the availability or accuracy of this data.` },
    ...
    { title: '7. Disclaimers', content: `${APP_NAME} is provided "as is" without warranties of any kind. We do not guarantee uptime, data accuracy, or uninterrupted service.\n\nThis is a free community project. We are not liable for any damages arising from your use of the platform.` },
  ...
  ru: [
    { title: '1. Принятие условий', content: `Используя ${APP_NAME}, вы соглашаетесь с настоящими Условиями использования. Если вы не согласны, пожалуйста, не используйте платформу.\n\n${APP_NAME} — это бесплатная платформа для игроков DDraceNetwork. Настоящие условия регулируют использование сайта и всех связанных сервисов.` },
    ...
    { title: '4. Пользовательский контент', content: `Вы сохраняете права собственности на создаваемый вами контент (посты на форуме, статьи и т.д.). Публикуя контент, вы предоставляете ${APP_NAME} неисключительную лицензию на его отображение и распространение на платформе.\n\nМы оставляем за собой право удалять контент, нарушающий настоящие условия или правила сообщества.` },
    { title: '5. Сторонние сервисы', content: `${APP_NAME} интегрируется с сервисами DDraceNetwork (ddnet.org) для получения данных об игроках, статистики и отрисовки скинов. Мы не являемся аффилированными лицами и не одобрены командой DDNet.\n\nСтатистика игроков и данные скинов получаются из публичных API DDNet. Мы не контролируем доступность и точность этих данных.` },
    ...
    { title: '7. Отказ от ответственности', content: `${APP_NAME} предоставляется «как есть» без каких-либо гарантий. Мы не гарантируем бесперебойную работу, точность данных или непрерывность сервиса.\n\nЭто бесплатный общественный проект. Мы не несём ответственности за любой ущерб, возникший в результате использования платформы.` },
  ...
  uk: [
    { title: '1. Прийняття умов', content: `Використовуючи ${APP_NAME}, ви погоджуєтесь з цими Умовами використання. Якщо ви не згодні — будь ласка, не користуйтесь платформою.\n\n${APP_NAME} — це безкоштовна платформа спільноти для гравців DDraceNetwork. Ці умови регулюють використання вебсайту та всіх пов'язаних сервісів.` },
    ...
    { title: '4. Контент користувачів', content: `Ви залишаєте за собою право власності на створений вами контент (пости на форумі, статті тощо). Публікуючи контент, ви надаєте ${APP_NAME} невиключну ліцензію на його відображення та розповсюдження на платформі.\n\nМи залишаємо за собою право видаляти контент, що порушує ці умови або правила спільноти.` },
    { title: '5. Сторонні сервіси', content: `${APP_NAME} інтегрується з сервісами DDraceNetwork (ddnet.org) для отримання даних гравців, статистики та відображення скінів. Ми не є афілійованими з командою DDNet та не маємо їхнього офіційного схвалення.\n\nСтатистика гравців та дані скінів отримуються з публічних API DDNet. Ми не контролюємо доступність та точність цих даних.` },
    ...
    { title: '7. Відмова від відповідальності', content: `${APP_NAME} надається «як є», без будь-яких гарантій. Ми не гарантуємо безперебійну роботу, точність даних або постійну доступність сервісу.\n\nЦе безкоштовний проєкт спільноти. Ми не несемо відповідальності за будь-які збитки, пов'язані з використанням платформи.` },
  ...
  de: [
    { title: '1. Annahme der Bedingungen', content: `Durch den Zugriff auf und die Nutzung von ${APP_NAME} erklären Sie sich mit diesen Nutzungsbedingungen einverstanden. Wenn Sie nicht einverstanden sind, nutzen Sie die Plattform bitte nicht.\n\n${APP_NAME} ist eine kostenlose Community-Plattform für DDraceNetwork-Spieler. Diese Bedingungen regeln Ihre Nutzung der Website und aller damit verbundenen Dienste.` },
    ...
    { title: '4. Nutzerinhalte', content: `Sie behalten das Eigentum an von Ihnen erstellten Inhalten (Forenbeiträge, Artikel usw.). Mit dem Veröffentlichen von Inhalten gewähren Sie ${APP_NAME} eine nicht-exklusive Lizenz zur Anzeige und Verbreitung dieser Inhalte auf der Plattform.\n\nWir behalten uns das Recht vor, Inhalte zu entfernen, die gegen diese Bedingungen oder unsere Community-Regeln verstoßen.` },
    { title: '5. Drittanbieterdienste', content: `${APP_NAME} ist mit DDraceNetwork-Diensten (ddnet.org) für Spielerdaten, Statistiken und Skin-Darstellung integriert. Wir sind weder mit dem DDNet-Team verbunden noch von diesem unterstützt.\n\nSpielerstatistiken und Skin-Daten werden von öffentlichen DDNet-APIs abgerufen. Wir haben keinen Einfluss auf die Verfügbarkeit oder Genauigkeit dieser Daten.` },
    ...
    { title: '7. Haftungsausschluss', content: `${APP_NAME} wird ohne jegliche Gewährleistung zur Verfügung gestellt. Wir garantieren weder Verfügbarkeit, Datengenauigkeit noch unterbrechungsfreien Betrieb.\n\nDies ist ein kostenloses Community-Projekt. Wir haften nicht für Schäden, die aus Ihrer Nutzung der Plattform entstehen.` },
  ...
  zh: [
    { title: '1. 条款接受', content: `访问和使用 ${APP_NAME} 即表示您同意受这些服务条款的约束。如果您不同意，请勿使用本平台。\n\n${APP_NAME} 是一个面向 DDraceNetwork 玩家的免费社区平台。这些条款管辖您对网站及所有相关服务的使用。` },
    { title: '2. 用户账户', content: '您可以创建账户以使用平台功能。您有责任维护账户安全，并对账户下的所有活动负责。\n\n注册时请提供准确信息。每人只能拥有一个账户，不允许共享账户。' },
    { title: '3. 用户行为', content: '您同意不会：\n• 将平台用于任何非法目的\n• 骚扰、威胁或辱骂其他用户\n• 试图利用漏洞或系统缺陷\n• 未经许可使用自动化工具访问平台\n• 冒充其他用户或 DDNet 工作人员\n• 以任何方式操纵游戏数据或作弊' },
    { title: '4. 用户内容', content: `您保留所创建内容（论坛帖子、文章等）的所有权。发布内容即表示您授予 ${APP_NAME} 在平台上展示和分发该内容的非独占许可。\n\n我们保留删除违反这些条款或社区规则的内容的权利。` },
    { title: '5. 第三方服务', content: `${APP_NAME} 集成了 DDraceNetwork 服务（ddnet.org）以获取玩家数据、统计信息和皮肤渲染。我们与 DDNet 团队没有附属或背书关系。\n\n玩家统计和皮肤数据从公开的 DDNet API 获取。我们无法控制这些数据的可用性或准确性。` },
    { title: '6. 账户终止', content: '如果您违反这些条款，我们可能随时暂停或终止您的账户。您可以随时通过设置页面删除您的账户。\n\n账户终止后，您使用平台的权利立即终止。' },
    { title: '7. 免责声明', content: `${APP_NAME} 按「原样」提供，不提供任何形式的保证。我们不保证正常运行时间、数据准确性或不间断服务。\n\n这是一个免费的社区项目。对于因使用本平台而产生的任何损失，我们不承担责任。` },
    { title: '8. 条款变更', content: '我们可能会不时更新这些条款。变更后继续使用平台即表示接受新条款。\n\n重大变更将通过平台的通知系统公布。' },
  ],
  tr: [
    { title: '1. Koşulların Kabulü', content: `${APP_NAME}'a erişerek ve kullanarak bu Kullanım Koşullarına bağlı kalmayı kabul etmiş olursunuz. Kabul etmiyorsanız lütfen platformu kullanmayın.\n\n${APP_NAME}, DDraceNetwork oyuncuları için ücretsiz bir topluluk platformudur. Bu koşullar, web sitesi ve ilgili tüm hizmetlerin kullanımını düzenler.` },
    ...
    { title: '4. Kullanıcı İçeriği', content: `Oluşturduğunuz içeriklerin (forum gönderileri, makaleler vb.) mülkiyeti size aittir. İçerik yayınlayarak ${APP_NAME}'a platformda gösterme ve dağıtma konusunda münhasır olmayan bir lisans vermiş olursunuz.\n\nBu koşulları veya topluluk kurallarımızı ihlal eden içerikleri kaldırma hakkımız saklıdır.` },
    { title: '5. Üçüncü Taraf Hizmetler', content: `${APP_NAME}, oyuncu verileri, istatistikler ve görünüm oluşturma için DDraceNetwork hizmetleriyle (ddnet.org) entegre çalışır. DDNet ekibi tarafından desteklenmemekte veya onaylanmamaktayız.\n\nOyuncu istatistikleri ve görünüm verileri herkese açık DDNet API'lerinden alınmaktadır. Bu verilerin erişilebilirliğini veya doğruluğunu kontrol etmemiz mümkün değildir.` },
    ...
    { title: '7. Sorumluluk Reddi', content: `${APP_NAME} herhangi bir garanti olmaksızın "olduğu gibi" sunulmaktadır. Kesintisiz hizmet, veri doğruluğu veya sürekli erişim garantisi vermiyoruz.\n\nBu ücretsiz bir topluluk projesidir. Platformu kullanımınızdan doğabilecek herhangi bir zarardan sorumlu değiliz.` },
